...
namespace NovellaEngine.Editor
{
    public class NovellaTextEditorWindow : EditorWindow
    {
        ...
        public static void OpenWindow(LocalizedString targetString, SerializedProperty fontSizeProp, NovellaTree tree, Action onSaved)
        {
            var win = GetWindow<NovellaTextEditorWindow>(true, ToolLang.Get("📝 Novella Text Editor", "📝 Редактор текста"), true);
            win._targetString = targetString;
        ...
        public void BuildUI()
        {
            ...
            var header = new Label(ToolLang.Get("📝 Novella Advanced Text Editor (Macro Compiler)", "📝 Продвинутый Редактор Текста (Макросы)")) { style = { backgroundColor = new Color(0.2f, 0.2f, 0.2f), color = Color.white, unityTextAlign = TextAnchor.MiddleCenter, paddingBottom = 5, paddingTop = 5, unityFontStyleAndWeight = FontStyle.Bold } };
