...
namespace NovellaEngine.Editor
{
    ...
    public class NovellaEdgeConnectorListener : IEdgeConnectorListener
    {
        ...
        public void OnDropOutsidePort(Edge edge, Vector2 position)
        {
            ...
            menu.AddItem(new GUIContent(ToolLang.Get("Dialogue Node", "Базовая Нода")), false, () => _graphView.CreateNode(localPos, ENodeType.Dialogue, port));
    ...
    public class NovellaGraphView : GraphView
    {
            ...
            if (change.elementsToRemove != null || change.edgesToCreate != null || change.movedElements != null)
            {
                ...
                if (change.elementsToRemove != null)
                {
                    foreach (var element in change.elementsToRemove)
                    {
                        ...
                        if (element is Edge edge)
                        {
                            ...
                            if (outNode != null)
                            {
                                if (outNode.Data.NodeType == ENodeType.Dialogue || outNode.Data.NodeType == ENodeType.Event) outNode.Data.NextNodeID = "";
                ...
                if (change.edgesToCreate != null)
                {
                    foreach (var edge in change.edgesToCreate)
                    {
                        ...
                        {
                            if (outNode.Data.NodeType == ENodeType.Dialogue || outNode.Data.NodeType == ENodeType.Event) outNode.Data.NextNodeID = inNode.Data.NodeID;
                            else if (outNode.Data.NodeType == ENodeType.Branch)
                            {
                                int portIdx = outNode.outputContainer.IndexOf(edge.output);
                ...
                EditorApplication.delayCall += () => { if (this != null && Tree != null) { SyncGraphToData(); Window.MarkUnsaved(); } };
            }
            return change;
        }
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            evt.menu.AppendAction($"Node/{ToolLang.Get("Dialogue Node", "Базовая Нода")}", (a) => CreateNode(pos, ENodeType.Dialogue));
        ...
        public void CreateNode(Vector2 position, ENodeType type, Port autoConnectPort = null)
        {
            ...
            if (autoConnectPort != null && autoConnectPort.node is NovellaNodeView parentNode)
            {
                ...
                nodeData.Speaker = pData.Speaker;
                nodeData.Mood = pData.Mood;
                nodeData.FontSize = pData.FontSize;
            }
            ...
            if (autoConnectPort != null && view.InputPort != null && type != ENodeType.Character)
            {
                if (autoConnectPort.capacity == Port.Capacity.Single && autoConnectPort.connected) DeleteElements(autoConnectPort.connections);
                AddElement(autoConnectPort.ConnectTo(view.InputPort));
            }
            Window.MarkUnsaved();
        }
        ...
        public override List<Port> GetCompatiblePorts(Port startPort, NodeAdapter nodeAdapter)
        {
            ...
            ports.ForEach(port => { if (startPort != port && startPort.node != port.node && startPort.direction != port.direction) compPorts.Add(port); });
            return compPorts;
        }
        ...
        public void LoadGraph()
        {
            ...
            if (!string.IsNullOrEmpty(Tree.RootNodeID) && nodeDict.TryGetValue(Tree.RootNodeID, out var rootNodeView))
            {
                ...
                if (startP != null && rootNodeView.InputPort != null) AddElement(startP.ConnectTo(rootNodeView.InputPort));
            }
            ...
            foreach (var nodeView in nodeDict.Values)
            {
                ...
                if (nodeView.Data.NodeType == ENodeType.Branch)
                {
                    ...
                    for (int i = 0; i < nodeView.Data.Choices.Count; i++)
                    {
                        ...
                        {
                            if (targetNode.InputPort != null) AddElement(ports[i].ConnectTo(targetNode.InputPort));
                        }
                    }
                }
                else
                {
                    if (!string.IsNullOrEmpty(nodeView.Data.NextNodeID) && nodeDict.TryGetValue(nodeView.Data.NextNodeID, out var targetNode))
                        if (nodeView.OutputPort != null && targetNode.InputPort != null) AddElement(nodeView.OutputPort.ConnectTo(targetNode.InputPort));
                }
            }
        }
    }
