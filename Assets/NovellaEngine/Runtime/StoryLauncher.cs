    ...
    public class StoryLauncher : MonoBehaviour
    {
        ...
        private void AutoWireButtons()
        {
            if (MainMenuPanel != null)
            {
                ...
                {
                    // Если на кнопке висит NovellaUIBinding — действие живёт там,
                    // в ClickSequence. Этот binding сам цепляется к onClick через
                    // OnEnable. Авто-AutoWire по имени НЕ нужен — иначе будет
                    // двойной обработчик. Это переезд с имени-AutoWire на
                    // binding-based (устойчиво к переименованию).
                    if (btn.GetComponent<NovellaEngine.Runtime.UI.NovellaUIBinding>() != null) continue;

                    btn.onClick.RemoveAllListeners();
                    string btnName = btn.name;
                    // Fallback по именам — для пользователей которые НЕ
                    // используют NovellaUIBinding на своих кнопках.
