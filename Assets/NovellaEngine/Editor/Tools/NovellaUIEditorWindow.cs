...
namespace NovellaEngine.Editor
{
    public class NovellaUIEditorWindow : EditorWindow
    {
        ...
        {
            if (_currentTab != 0 && _currentTab != 2) { if (_tempChoicePreview) DestroyImmediate(_tempChoicePreview); }
            if (_currentTab != 1 && _currentTab != 2) { if (_tempStoryPreview) DestroyImmediate(_tempStoryPreview); }
        ...
        private void GenerateMCCreationPanel(Transform rootTransform)
        {
            ...
            // Стрелки переключения персонажей
        ...
        private void DrawWardrobeSection(Canvas canvas)
        {
            ...
            else
            {
                ...
                if (uiScript != null)
                {
                    ...
                    // Убрано управление префабом вкладок, так как они теперь свободные объекты
                    GUILayout.EndVertical();
                ...
                if (isEditingWardrobe)
                {
                    foreach (Transform child in wardrobeTransform)
                    {
                        ...
                        else if (rt != null && !rt.name.StartsWith("Tab_")) // Скрываем табы из общего списка, так как для них есть свой блок
                        {
        ...
        private void GenerateWardrobePanel(Transform rootTransform)
        {
            ...
            DestroyImmediate(tempSlot);

            // ИСПРАВЛЕНО: Спавним ФУНКЦИОНАЛЬНЫЕ кнопки вкладок
            ...
            // === АВТО-ПРИВЯЗКА СКРИПТА И ССЫЛОК ===
        ...
        private void DrawGameplayUITab()
        {
            ...
            // ИСПРАВЛЕНО: Добавлено предупреждение о переопределении графа
        ...
        private void DrawChoiceButtonPrefabEditor()
        {
            if (_tempChoicePreview == null && _player.ChoiceContainer != null)
            {
                ...
                // УБРАЛИ СТРОЧКУ _activeEditRect = ... ИМЕННО ОНА КРАЛА ФОКУС
            }
        ...
        private void DrawTMPTextSettingsBlock(TMP_Text txt, string title)
        {
            ...
            // ИСПРАВЛЕНО CS0117: Используем стандартное поле Unity вместо Галереи
