...
{
    public class NovellaVariableEditorWindow : EditorWindow
    {
        ...
        // === ОБНОВЛЕНО ДЛЯ ПРИЕМА ПЕРЕМЕННОЙ ===
        public static void ShowWindow(string variableToSelect = null)
        {
            var win = GetWindow<NovellaVariableEditorWindow>(ToolLang.Get("Global Variables", "База Переменных"));
            win.minSize = new Vector2(750, 600);
            win.Show();

            if (!string.IsNullOrEmpty(variableToSelect))
            {
                var settings = NovellaVariableSettings.Instance;
                int idx = settings.Variables.FindIndex(v => v.Name == variableToSelect);
                if (idx != -1) win._selectedIndex = idx;
            }
        }
