        ...
        private static void DrawIcon(Texture2D tex, Icon icon)
        {
            ...
            switch (icon)
            {
                ...
                case Icon.Console:
                    ...
                    break;
                case Icon.Settings:
                    // Шестерёнка, упрощённая под downscale до 16×16.
                    // Стратегия: одиночные толстые штрихи-зубцы радиально.
                    // Без двойных линий — на маленьком размере они сливались
                    // в неразборчивое пятно. Контур + центральная дырка дают
                    // силуэт, 8 одиночных штрихов — зубцы.
                    DrawCircle(tex, 16, 16, 7);   // тело (поменьше — освободить место под зубцы)
                    DrawCircle(tex, 16, 16, 2);   // дырка по центру

                    // 4 кардинальных зубца (вертикальные/горизонтальные).
                    DrawLine(tex, 16, 4,  16, 8);   // верх
                    DrawLine(tex, 16, 24, 16, 28);  // низ
                    DrawLine(tex, 4,  16, 8,  16);  // лево
                    DrawLine(tex, 24, 16, 28, 16);  // право

                    // 4 диагональных зубца.
                    DrawLine(tex, 7,  7,  10, 10);  // ↖
                    DrawLine(tex, 22, 22, 25, 25);  // ↘
                    DrawLine(tex, 7,  25, 10, 22);  // ↙
                    DrawLine(tex, 22, 10, 25, 7);   // ↗
                    break;
            }
        }
