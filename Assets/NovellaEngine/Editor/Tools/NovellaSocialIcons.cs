...
namespace NovellaEngine.Editor
{
    public static class NovellaSocialIcons
    {
        ...
        // квадрат + белый силуэт «геймпада» внутри. Силуэт состоит из:
        //   • основного тела (горизонтально вытянутый закруглённый rect),
        //   • двух «ножек» снизу-слева и снизу-справа — характерные хвостики
        //     которые делают форму похожей на джойстик,
        //   • двух овалов-глаз цвета фона.
        private static Texture2D BuildDiscord()
        {
            var t = MakeTex();

            // 1. Фон — фиолетовый закруглённый квадрат.
            FillRoundedRect(t, 4, 4, 60, 60, 14, DiscordBlurple);

            // 2. Основное тело силуэта — белый закруглённый прямоугольник
            // в верхней половине, чуть вытянут по горизонтали.
            FillRoundedRect(t, 14, 26, 50, 50, 12, Color.white);

            // 3. Две «ножки» снизу — закруглённые «языки» торчащие из тела
            // вниз и слегка наружу. Делаем кругами + соединительной полоской.
            // Левая:
            FillCircle(t, 21, 14, 6f, Color.white);
            FillRoundedRect(t, 17, 14, 25, 30, 4, Color.white);
            // Правая:
            FillCircle(t, 43, 14, 6f, Color.white);
            FillRoundedRect(t, 39, 14, 47, 30, 4, Color.white);

            // 4. Глаза — две вертикально-вытянутые «капельки» цветом фона.
            // Через две перекрывающиеся окружности.
            FillCircle(t, 24, 36, 3.2f, DiscordBlurple);
            FillCircle(t, 24, 38, 3.2f, DiscordBlurple);
            FillCircle(t, 40, 36, 3.2f, DiscordBlurple);
            FillCircle(t, 40, 38, 3.2f, DiscordBlurple);
